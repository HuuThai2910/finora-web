...
export interface AiRule {
  ...
  nhom_5c: string;
  mo_ta: string;
  /** True nghĩa là giá trị càng thấp càng tốt (ví dụ DTI). */
  nghich_dao: boolean;
  /** True thì luật tra bảng theo giá trị rời rạc thay vì so ngưỡng. */
  la_bang_diem: boolean;
  bat: boolean;
...
export interface AiRuleUpdate {
  ...
  diem_khi_thieu: number;
  bac?: RuleBac[];
  bang_diem?: Record<string, number>;
}

export interface AiRulesUpdate {
  rules: Record<string, AiRuleUpdate>;
}
...
/** Vết của một luật 5C đã chạy — cơ sở giải trình của Rule Engine (D4). */
